...
async function generateRoflVideo(): Promise<Blob> {
  ...
  return new Promise((resolve) => {
    ...
    const animate = () => {
      ...
      // 챔피언들 그리기 (더미 데이터)
      const champions = [
        { name: '이즈리얼', x: 200 + Math.sin(frame * 0.1) * 100, y: 200 + Math.cos(frame * 0.1) * 100, color: '#ff6b6b' },
        { name: '세라핀', x: 400 + Math.sin(frame * 0.15) * 80, y: 300 + Math.cos(frame * 0.15) * 80, color: '#4ecdc4' },
        { name: '리신', x: 600 + Math.sin(frame * 0.2) * 120, y: 400 + Math.cos(frame * 0.2) * 120, color: '#45b7d1' }
      ]
      ...
      if (frame % 60 === 0) { // 2초마다
        ...
        ctx.font = '16px Arial'
        ctx.fillText('⚔️ 팀파이트 발생!', canvas.width - 200, 50)
      }
